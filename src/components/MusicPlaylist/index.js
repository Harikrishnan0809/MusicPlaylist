...
class MusicPlaylist extends Component {
  ...
  onDeleteList = id => {
    this.setState(preVal => ({
      tracksList: preVal.tracksList.filter(each => each.id !== id),
    }))
  }
  ...
  render() {
    ...
    const itIsHave = filteredList.length === 0
    return (
      <MusicPlaylistContainer>
        <TopContainer>
          <NameHeading>Ed Sheeran</NameHeading>
          <RoleName>Singer</RoleName>
        </TopContainer>

        <BottomContainer>
          <HeadingSearchContainer>
            <SongHeading>Songs Playlist</SongHeading>
            <InputContainer>
              <SearchInput
                type="search"
